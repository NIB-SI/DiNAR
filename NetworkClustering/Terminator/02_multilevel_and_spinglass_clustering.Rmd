...
5. infomap.community (w,d) - in my case, works only on undirected, simplified (loops and multiple relations are removed) LKN with 128 (2^7) iterations and set.seed()
...
<font color='MidnightBlue'>1. Cluster LKN using Multi-Level algorithm (https://arxiv.org/abs/0803.0476)</font>
...
myname = "terminator.graphml"#sort(list.files(path = dir1,pattern = "\\.graphml$"))
print(myname)
...
<font color='purple'>On undirected, simplified network (LKN)</font>
...
write.table(mydf, paste0(dir1, '/mydf.txt'), sep='\t', row.names = FALSE, quote = FALSE)
...
if (cnt) {
  mini = induced.subgraph(graph=g,vids=strtoi(Vid))
...
if(length(IND)){
  for (i in 1:length(IND)){
      ...
      moderateNet = induced.subgraph(graph=g,vids=strtoi(cl[[IND[i]]]))
      moderateNetGS = induced.subgraph(graph=gs,vids=strtoi(cl[[IND[i]]]))
      # moderateNetGSS = delete.vertices(moderateNetGS, which(degree(moderateNetGS)<2))
    ...
    if ((ecount(moderateNet) < 2^6) & (vcount(moderateNet) < 2^6)) {
      ...
      if(vcount(moderateNet) >= 2^2) {
        ...
        plot(moderateNet, layout = l2,
             ...
             vertex.size = V(moderateNet)$size,
...
if(length(IND)){
  for (i in 1:length(IND)){
    ...
    starNet = induced.subgraph(graph=g,vids=strtoi(cl[[IND[i]]]))
    ...
    #     print (cbind(V(starNet)$nodeID[get.edgelist(starNet)[duplicated(get.edgelist(starNet)),1]],
    #                                          V(starNet)$nodeID[get.edgelist(starNet)[duplicated(get.edgelist(starNet)),2]]))
    cat('\n')
...
if(length(IND)){
  for (i in 1:length(IND)){
    ...
    denselyConnectedNet = induced.subgraph(graph=g,vids=strtoi(cl[[IND[i]]]))
    ...
    if((vcount(denselyConnectedNet) >= 2^2) & (ecount(denselyConnectedNet) <= 2^11)) {
      ...
      plot(denselyConnectedNet, layout = l2,
           ...
           vertex.size = V(denselyConnectedNet)$size,
           rescale=FALSE, add=TRUE,
           vertex.label.cex= 0.5,
           edge.arrow.size = 0.25,
           edge.arrow.width = 0.25,
...
if(length(IND)){
  for (i in 1:length(IND)){
    ...
    unConnectedNet = induced.subgraph(graph=g, vids=strtoi(cl[[IND[i]]]))

    mydegSC = degree(unConnectedNet, loops = FALSE, normalized = FALSE,  mode = "all")
    mydegSC = cbind(strtoi(cl[[IND[i]]]), mydegSC)
    ...
    unConnectedNetLeftovers = graphs[[which(unlist(lapply(graphs,
                                                          function(x) vcount(x)))
                                            !=
                                              max(unlist(lapply(graphs,
                                                                function(x) vcount(x)))))]]

    if ((length(graphs)-1) != 1){
      vids = unlist(sapply(unConnectedNetLeftovers,function(x) (V(x)$numVec)))
    ...
    cat("#edges/#vertices", 
        ...
        "\n")
    #     cat ('duplicated connect rows:', '\n')
    #     print (cbind(V(unConnectedNetLeftovers)$nodeID[get.edgelist(unConnectedNetLeftovers)[duplicated(get.edgelist(unConnectedNetLeftovers)),1]],
    #                                          V(unConnectedNetLeftovers)$nodeID[get.edgelist(unConnectedNetLeftovers)[duplicated(get.edgelist(unConnectedNetLeftovers)),2]]))
    cat('\n')
    ...
    if((vcount(unConnectedNetLeftovers) >= 2^2) & (ecount(unConnectedNetLeftovers) <= 2^11)) {
      ...
      plot(unConnectedNetLeftovers, layout = l2,
           ...
           vertex.size = V(unConnectedNetLeftovers)$size,
           rescale=FALSE, add=TRUE,
           vertex.label.cex= 0.5,
           edge.arrow.size = 0.25,
           edge.arrow.width = 0.25,
    ...
    for (j in df[,1]){
      cat('##########\n')
      mysubg = induced.subgraph(graph=unConnectedNet,vids=clspin[[j]])
      ...
      if((vcount(mysubg) >= 2^2) & (ecount(mysubg) <= 2^10)) {
        plot(0, type="n", ann=TRUE, axes=FALSE,
             xlim=extendrange(l2[,1]),
             ylim=extendrange(l2[,2]),
...
save.image(paste0(dir1, '/pt1.RData'))
# load('pt1.RData')
...
if(length(IND)){
  for (i in 1:length(IND)){
    ...
    cat('i = ', i, "\n")
    ind = strtoi(cl[[i]]) # node ids
    entryGraph = induced.subgraph(graph=g,vids=strtoi(cl[[IND[i]]]))
    ...
    for (j in df[,1]){
      cat('##########\n')
      mysubg = induced.subgraph(graph=entryGraph,vids=clspin[[j]])
...
save.image(paste0(dir1, '/pt2.RData'))
# load('pt2.RData')
...
(all(mat[,1] %in% V(g)$nodeID))
(all(V(g)$nodeID %in% mat[,1]))
...
write.table(mat.sorted, paste0(myname,"_id_coord_clu.tsv"), 
...
setdiff(V(g)$nodeID, mat.sorted$nodeID)
...
E(g)$cluA = V(g)$membership[match(E(g)$ID1,  V(g)$nodeID)]
E(g)$cluB = V(g)$membership[match(E(g)$ID2,  V(g)$nodeID)]
E(g)$superCluA = V(g)$superClu[match(E(g)$ID1,  V(g)$nodeID)]
E(g)$superCluB = V(g)$superClu[match(E(g)$ID2,  V(g)$nodeID)]
E(g)$cluOriginA = V(g)$cluOrigin[match(E(g)$ID1,  V(g)$nodeID)]
E(g)$cluOriginB = V(g)$cluOrigin[match(E(g)$ID2,  V(g)$nodeID)]
E(g)$matchClu = (E(g)$cluA == E(g)$cluB)
E(g)$degreeFullSimplifiedA = V(g)$degreeFullSimplified[match(E(g)$ID1,  V(g)$nodeID)]
E(g)$degreeFullSimplifiedB = V(g)$degreeFullSimplified[match(E(g)$ID2,  V(g)$nodeID)]
E(g)$degreeSimplifiedSuperClusterA = V(g)$degreeSimplifiedSuperCluster[match(E(g)$ID1,  V(g)$nodeID)]
E(g)$degreeSimplifiedSuperClusterB = V(g)$degreeSimplifiedSuperCluster[match(E(g)$ID2,  V(g)$nodeID)]
E(g)$degreeSimplifiedClusterA = V(g)$degreeSimplifiedCluster[match(E(g)$ID1,  V(g)$nodeID)]
E(g)$degreeSimplifiedClusterB = V(g)$degreeSimplifiedCluster[match(E(g)$ID2,  V(g)$nodeID)]
list.edge.attributes(g)
...
# which columns!!!???!!!
colNames = toupper(c('nodeID', 'shortDescription', 'shortName', 'MapManBin', 
...
write.table(df1, paste0(myname, "_NODES.tsv"),
            quote = FALSE,
            row.names = FALSE,
            sep = "\t")
...
# which columns!!!???!!!
colNames = toupper(c('ID1', 'ID2', 'type', 
                     'cluA', 'cluB', 
                     'superCluA', 'superCluB', 
...
write.table(df2, paste0(myname, "_EDGES_all.tsv"), 
            quote = FALSE,
            row.names = FALSE,
            sep = "\t")
...
write.table(df3, paste0(myname, "_EDGES_byClu.tsv"), 
            quote = FALSE,
            row.names = FALSE,
            col.names = TRUE,
            sep = "\t")
...
save.image(paste0(myname,'_clu_and_coord.RData'))
