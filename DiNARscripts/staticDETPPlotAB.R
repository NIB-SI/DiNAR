...
plot.net2 <- function(node1, edge1, clusterID, isOK){
  ...
  if (!isOK) {
    ...
    newplot(xlim=c(range(n$x)[1], range(n$x)[2]+8),ylim=range(n$y),pty="m", asp=12/16)
    ...
    # # Background
    # vertices
    points(n[,"x"],n[,"y"],col=bkg,cex=1.5)
    # edges
    segments(
      n[e$geneID1,"x"],
      n[e$geneID1,"y"],
    ...
    # Background
    # inner part
    points(n$x,n$y,col=bkg, pch=16, cex=0.75) # Background
    #.tst <- TRUE
    #print(c(day,t))
