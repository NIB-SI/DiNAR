...
plot.net <- function(node1, edge1, clusterID, isOK){
  ...
  if (!isOK) {
    ...
    abs1 = abs(n[e$geneID1,exprCol][sel])
    max1 = max(abs1, 1)
    abs2 = abs(n[e$geneID2,exprCol][sel])
    max2 = max(abs2, 1)
    ...
    sidePalette <- data.frame(color = rev(unique(isolate(mypalette())$palette)), 
    ...
    vn %>%
      ...
      visOptions(highlightNearest = list(enabled = TRUE, hover = FALSE, degree = 1, algorithm = "all", hideColor = 'rgba(255,255,255,0)'),
      ...
      visInteraction(navigationButtons = TRUE,
                     keyboard = TRUE, 
                     tooltipDelay = 0) %>%
      visLegend(useGroups = FALSE, addEdges = tmpdf, addNodes = sidePalette, position = 'right', stepX = 100, stepY = 25, width = 0.2, ncol = 1) %>%
